...
void MainLoopController::processMenubar()
{
    ...
    AlienImGui::MenuItem(
        AlienImGui::MenuItemParameters().name("自动保存").keyAlt(true).key(ImGuiKey_6).selected(AutosaveWindow::get().isOn()).closeMenuWhenItemClicked(false),
        [&] { AutosaveWindow::get().setOn(!AutosaveWindow::get().isOn()); });
    AlienImGui::MenuItem(
        AlienImGui::MenuItemParameters().name("日志").keyAlt(true).key(ImGuiKey_7).selected(LogWindow::get().isOn()).closeMenuWhenItemClicked(false),
